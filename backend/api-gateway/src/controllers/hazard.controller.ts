...
export const getHazards = (req: Request, res: Response) => {
  const { hazard_type, hazard_severity, hazard_status, page, limit } =
    req.query;

  const grpcRequest = {
    hazard_type: (hazard_type as string) || "",
    hazard_severity: (hazard_severity as string) || "",
    hazard_status: (hazard_status as string) || "",
