...
// Helper input shapes.
// ---------------------------------------------------------------------------
// Aligned to the live UserRole enum in backend/libs/common/src/constant/user-role.ts
// (Phase 0 discovery, 28 Jul 2026) rather than the originally designed TEAVS role set
// (council_officer / public_user / emergency_services), since that set does not exist
// in the running Phoenix codebase.
// The known roles keep editor autocomplete and document the intended set, while
// `(string & {})` still admits values that exist in real data but not in the
// enum -- the live `user_account` table contains the role "ingestion service"
// (with a space) rather than the enum's "ingestion_service". A log record must
// report the role that was actually presented; narrowing or casting here would
// hide exactly the kind of mismatch this logging is meant to surface. Same
// idiom as `SecurityRule` and `ComponentName` above.
export type Role =
  | 'admin'
  | 'analyst'
  | 'end_user'
